...
void WaveFrontOBJSerializer::writeHeader() {
	obj_stream << "# File generated by IfcOpenShell " << IFCOPENSHELL_VERSION << "\n# master branch commit on Jun 28, 2016 hash: 240133515abb233deff895efaf91e495ff75afa9\n";
#ifdef WIN32
...
void WaveFrontOBJSerializer::write(const IfcGeom::TriangulationElement<real_t>* o)
{
	...
	obj_stream << std::setprecision(std::numeric_limits<real_t>::digits10);
    ...
    for ( std::vector<real_t>::const_iterator it = mesh.verts().begin(); it != mesh.verts().end(); ) {
		...
		obj_stream << "v " << std::fixed << std::setprecision(8) << x << " " << std::fixed << std::setprecision(8) << y << " " << std::fixed << std::setprecision(8) << z << "\n";
	}
    ...
    for ( std::vector<real_t>::const_iterator it = mesh.normals().begin(); it != mesh.normals().end(); ) {
        ...
        const real_t y = *(it++);
        const real_t z = *(it++);
		obj_stream << "vn " << std::fixed << std::setprecision(8) << x << " " << std::fixed << std::setprecision(8) << y << " " << std::fixed << std::setprecision(8) << z << "\n";
	}
	...
	int previous_shape_counter_id = -2;
	std::vector<long int>::const_iterator shape_counter_it = mesh.shape_counter_ids().begin();
	...
	for ( std::vector<int>::const_iterator it = mesh.faces().begin(); it != mesh.faces().end(); ) {
		...
		// add an inter-shape comment.
		const long int shape_counter_id = *(shape_counter_it++);
		if (shape_counter_id != previous_shape_counter_id) {
			obj_stream << "# sub_obj: " << shape_counter_id << "\n"; //obj_stream << "usemtl " << material_name << "\n";

			previous_shape_counter_id = shape_counter_id;
		}
